...
RNN 是递归的（时刻  依赖 ），这导致无法并行计算。TDNN 本质上是**一维空洞卷积（1D Dilated Convolution）**。它通过“在此刻看过去和未来几个特定时刻”来聚合信息。

* **Context Splicing（上下文拼接）**：
* Layer 1: 看 ，感受野为 5。
* Layer 2: 看 Layer 1 的 （通常会有空洞，如 ）。
...
* **Chunk Size ()**：模型一次前向传播处理的时长（如 640ms, 1000ms）。
* **Inference Step**：每收到  毫秒数据，进行一次推理。

#### 2. State Carrying (状态传递)

对于 LSTM，时刻  的输出依赖于 。在流式中，当 Chunk  处理完后，必须将其最后一帧的 Hidden State () 和 Cell State () **缓存**下来，作为 Chunk  的初始状态。
...
* ：分块大小（如 40ms 一块，或者更大）。
* ：右侧前瞻（Right Context / Lookahead），即为了识别当前帧，必须等待的未来帧时长。
* ：模型推理计算耗时。
...
*解释：为了输出 Chunk 的第一个字，我至少要等这块读完（），并且等未来的信息都到齐（）。*
