...
* **影响**：导致能量计算（RMS）虚高，影响 VAD 的阈值判断；在做 FFT 时会在 0Hz 处产生巨大的能量尖峰。
...
* *对策*：要么将所有数据**降采样**到 8kHz 训练专用模型，要么训练一个带宽鲁棒的模型（Bandwidth Agnostic），在训练中随机对 16kHz 数据做 8kHz 低通滤波以模拟窄带。
...
1. **显存预算（OOM）**：Transformer 的 Self-Attention 显存消耗随长度平方增长（O(L^2)）。通常训练切片控制在 **30秒** 以内是性价比最高的。
...
* **原理**：重新采样音频。
* 播放速度 0.9x → 时长变长、基频降低（男声化）。
* 播放速度 1.1x → 时长变短、基频升高（女声化）。


* **数学本质**：时间轴缩放，t' = t / s（s 为速度因子）。
* **操作**：
...
* **原理**：y(t) = x(t) * h(t)，其中 h(t) 是房间冲激响应（Room Impulse Response）。
...
2. **SNR 计算**：如果你有一段能量为 E_s 的语音，你想加入一段噪声，使得 SNR 为 10dB。你需要将噪声的能量 E_n 缩放到 E_s 的多少倍？（Hint: 利用 SNR(dB)=10·log10(E_s/E_n) 公式反推）。
